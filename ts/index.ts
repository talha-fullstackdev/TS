// let a  = 67
// a = "talha"  // cannot assign a number type to string type in TS
/////////////// asigning types to variables
let myName:string = "Talha" // by this way we can added types to a variable
myName = "Talha Nawaz"
let myNum : number = 45
myNum = 45
////////////////////////////////
///////// asigning types to functions
const sum  = (a:number ,b:number)=>{ // by this way we can give types to functions parameters
    return a +b
}
const sum2 = (a:number,b:number):number => { // by this way we can assign types to functions
    return a+b
}